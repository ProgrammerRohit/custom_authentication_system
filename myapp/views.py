...
from .forms import ExtendedRegisterForm, ExtendedUpdateProfileForm, EmailAuthenticationForm
...
def loginView(req):
    if not req.user.is_authenticated:
        if req.method == 'POST':
            form_data = EmailAuthenticationForm(request=req, data=req.POST)
            if form_data.is_valid():
            ...
            form_data = EmailAuthenticationForm()
    else:
        return HttpResponseRedirect('/profile/')
    context = {
        'uform' : form_data
...
def profile(req):
    if req.user.is_authenticated:
     ...
     return render(req, template_name='myapp/profile.html', context=context)
    else:
...
def updateprofile(req):
    if req.user.is_authenticated:
        if req.method == "POST":
            form_data = ExtendedUpdateProfileForm(req.POST,req.FILES,instance=req.user)
            if form_data.is_valid():
                form_data.save()
                return HttpResponseRedirect('/profile/')
        else:
